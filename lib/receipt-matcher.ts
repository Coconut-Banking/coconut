...
/** Strip common POS terminal prefixes that obscure the real merchant name. */
...
export async function matchReceiptsToTransactions(
  ...
  const tightWindowDays = RECEIPT_MATCH.TIGHT_DATE_WINDOW_DAYS;
  ...
  for (const receipt of receipts) {
    ...
    // Build the list of amounts to try: total first, then subtotal if different
    ...
    // Try each amount candidate (total, then subtotal)
    ...
    if (!bestMatchId) {
      // Diagnostic: find the closest candidate to understand why matching failed
  ...
  if (debugUnmatched.length > 0 && process.env.NODE_ENV === "development") {
    console.log(`[receipt-matcher] ${debugUnmatched.length} unmatched receipts:`);
    for (const d of debugUnmatched.slice(0, 10)) {
      console.log(`  ${d.merchant} $${d.amount} (${d.date ?? "no date"}) — ${d.reason}`);
    }
    if (debugUnmatched.length > 10) {
      console.log(`  ... and ${debugUnmatched.length - 10} more`);
    }
  }
