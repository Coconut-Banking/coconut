...
export async function notifyGroupMembers(
  ...
  // Fetch group owner + members in parallel (independent queries)
  const [{ data: group }, { data: members }] = await Promise.all([
    db.from("groups").select("owner_id").eq("id", groupId).single(),
    db.from("group_members").select("user_id").eq("group_id", groupId).not("user_id", "is", null),
  ]);
