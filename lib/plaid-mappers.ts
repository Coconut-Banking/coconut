export const CATEGORY_COLORS: Record<string, string> = {
  ENTERTAINMENT: "bg-purple-100 text-purple-700",
  RESTAURANTS: "bg-orange-100 text-orange-700",
  GROCERIES: "bg-emerald-100 text-emerald-700",
  TRAVEL: "bg-cyan-100 text-cyan-700",
  TRANSPORTATION: "bg-blue-100 text-blue-700",
  SHOPPING: "bg-amber-100 text-amber-700",
  UTILITIES: "bg-gray-100 text-gray-700",
  HEALTHCARE: "bg-pink-100 text-pink-700",
  FITNESS: "bg-rose-100 text-rose-700",
  SUBSCRIPTIONS: "bg-violet-100 text-violet-700",
  STREAMING: "bg-violet-100 text-violet-700",
  PERSONAL_CARE: "bg-indigo-100 text-indigo-700",
  HAIRCUT: "bg-indigo-100 text-indigo-700",
  GENERAL_MERCHANDISE: "bg-amber-100 text-amber-700",
  GENERAL_SERVICES: "bg-slate-100 text-slate-700",
  FOOD_AND_DRINK: "bg-orange-100 text-orange-700",
  COFFEE: "bg-amber-100 text-amber-800",
  FAST_FOOD: "bg-orange-100 text-orange-700",
  ALCOHOL: "bg-red-100 text-red-700",
  HOME_IMPROVEMENT: "bg-teal-100 text-teal-700",
  RENT_AND_UTILITIES: "bg-gray-100 text-gray-700",
  GAMBLING: "bg-red-100 text-red-700",
  CANNABIS: "bg-lime-100 text-lime-700",
  GAS_AND_FUEL: "bg-sky-100 text-sky-700",
  PARKING: "bg-slate-100 text-slate-700",
  RIDESHARE: "bg-blue-100 text-blue-700",
  CLOTHING: "bg-fuchsia-100 text-fuchsia-700",
  ELECTRONICS: "bg-cyan-100 text-cyan-700",
  EDUCATION: "bg-indigo-100 text-indigo-700",
};

export const MERCHANT_COLORS = [
  "#E50914", "#1DB954", "#00674B", "#FF9900", "#003366", "#7BB848", "#555555",
  "#4A6CF7", "#E8507A", "#F59E0B", "#10A37F", "#FF5A5F", "#1A1A1A", "#4A90D9",
];

export function hashColor(str: string | null | undefined): string {
  if (!str) return MERCHANT_COLORS[0];
  let h = 0;
  for (let i = 0; i < str.length; i++) h = (h << 5) - h + str.charCodeAt(i);
  return MERCHANT_COLORS[Math.abs(h) % MERCHANT_COLORS.length];
}

export function fmtDate(dateStr: string | null | undefined): string {
  if (!dateStr) return "—";
  const d = new Date(dateStr + "T12:00:00");
  if (isNaN(d.getTime())) return "—";
  const months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
  return `${months[d.getMonth()]} ${d.getDate()}, ${d.getFullYear()}`;
}
