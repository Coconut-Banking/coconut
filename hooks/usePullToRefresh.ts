...
export function usePullToRefresh(onRefresh: () => void | Promise<void>, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    function createIndicator() {
      ...
      indicator.style.backgroundColor = "#F5F3F2";
      indicator.innerHTML = `
        <div class="flex items-center gap-2 text-sm text-[#1e2021]">
    ...
    function showIdle(ind: HTMLDivElement, pullPx: number) {
      ...
      ind.innerHTML = pullPx >= PULL_THRESHOLD
        ? `<div class="flex items-center gap-2 text-sm text-[#1e2021]">Release to refresh</div>`
