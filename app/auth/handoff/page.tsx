...
export default function AuthHandoffPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await (signIn as { create?: (p: { ticket: string }) => Promise<{ createdSessionId?: string }> }).create?.({ ticket } as { ticket: string });
        const sessionId = (result as { createdSessionId?: string })?.createdSessionId;
        if (cancelled) return;
