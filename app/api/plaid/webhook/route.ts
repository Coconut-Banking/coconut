...
export async function POST(request: NextRequest) {
  ...
  if (webhook_type === "TRANSACTIONS" && webhook_code === "SYNC_UPDATES_AVAILABLE") {
    ...
    if (webhook_code === "NEW_ACCOUNTS_AVAILABLE") {
      db.from("plaid_items").update({ new_accounts_available: true }).eq("plaid_item_id", item_id).then(() => {}).catch(() => {});
      syncTransactionsForUser(clerkUserId)
        .then((r) => {
      ...
      db.from("plaid_items").update({ needs_reauth: true }).eq("plaid_item_id", item_id).then(() => {}).catch(() => {});
      ...
      db.from("plaid_items").update({ needs_reauth: false }).eq("plaid_item_id", item_id).then(() => {}).catch(() => {});
