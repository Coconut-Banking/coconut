...
export default function SubscriptionsPage() {
  ...
  const { subscriptions: realSubs, totalMonthly: realTotal, totalAnnual: realAnnual, loading, detecting, detect, dismiss } = useSubscriptions();

  // Real subscriptions flow when linked and not demo
  if (linked && !isDemo) {
    return (
      <div className="max-w-3xl mx-auto px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {loading ? "Loading…" : `${realSubs.length} active · $${realTotal.toFixed(2)}/mo · $${realAnnual.toFixed(0)}/yr`}
          </p>
        </div>
        <button
          onClick={detect}
        ...
        >
          ...
          {detecting ? "Detecting…" : "Detect recurring charges"}
        </button>
        {loading ? (
          <div className="bg-gray-50 border border-gray-100 rounded-2xl p-12 text-center text-gray-500">Loading…</div>
        ) : realSubs.length === 0 ? (
          <div className="bg-gray-50 border border-gray-100 rounded-2xl p-12 text-center">
            ...
            <p className="text-xs text-gray-400">Connect your bank, sync transactions, then tap &quot;Detect recurring charges&quot; to find subscriptions.</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-white rounded-2xl border border-gray-100 p-4">
                ...
                <div className="text-xl font-bold text-gray-900">${realTotal.toFixed(2)}</div>
              </div>
              <div className="bg-white rounded-2xl border border-gray-100 p-4">
                <div className="text-xs text-gray-400 mb-1">Annual</div>
                <div className="text-xl font-bold text-gray-900">${realAnnual.toFixed(0)}</div>
              </div>
            </div>
              ...
              {realSubs.map((sub, i) => (
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    <div className="text-sm font-bold text-gray-900">${sub.amount.toFixed(2)}/{sub.frequency === "yearly" ? "yr" : "mo"}</div>
                  </div>
                  <button
