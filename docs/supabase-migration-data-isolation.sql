...
-- 1. Replace the global unique index on groups(source, external_id)
--    with a per-owner index.
DROP INDEX IF EXISTS groups_source_ext_idx;
DROP INDEX IF EXISTS groups_source_ext_owner_idx;
CREATE UNIQUE INDEX groups_source_ext_owner_idx
  ON groups(owner_id, source, external_id)
  WHERE external_id IS NOT NULL;

-- 2. Replace the global unique index on split_transactions(source, external_id)
--    with a per-group index (each user has their own group copy).
DROP INDEX IF EXISTS split_transactions_source_ext_idx;
DROP INDEX IF EXISTS split_transactions_source_ext_group_idx;
