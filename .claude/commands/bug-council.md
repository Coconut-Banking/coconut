...
After ALL 5 agents return, collect their findings and spawn ONE **Devil's Advocate** agent.
...
4. Run the test to verify it passes:
   ...
   ```
5. Stage all changed files (source fix + test) and commit:
   ```
...
7. If the bug does not exist as described (the Devil's Advocate missed it), respond with "SKIP: {reason}" and do NOT commit.
```
